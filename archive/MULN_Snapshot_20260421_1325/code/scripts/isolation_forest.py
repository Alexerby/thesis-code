...
# --- Feature Tiers ---
TIERS = {
    1: ["delta_t", "relative_size"],
    2: ["delta_t", "relative_size", "price_distance_ticks", "volume_ahead"],
    3: ["delta_t", "relative_size", "price_distance_ticks", "volume_ahead", "induced_imbalance", "spread_bps"],
    4: ["delta_t", "relative_size", "induced_imbalance"]
}

def load_and_preprocess(path: str, features: list, scaler=None) -> tuple[pd.DataFrame, StandardScaler]:
...
def parse_args():
    ...
    # Model persistence
    p.add_argument("--save-model", help="Path to save the trained ensemble and scaler (.joblib)")
    p.add_argument("--load-model", help="Path to load a saved ensemble and scaler (.joblib)")
    
    # Configuration
    p.add_argument("--tier", type=int, default=1, choices=[1, 2, 3, 4], help="Feature tier (default 1)")
    ...
    p.add_argument("--info", action="store_true", help="Display info about the loaded model and exit")
    return p.parse_args()
...
def main():
    ...
    if args.test:
        ...
        print(f"  Flagged: {anomalies.sum():,} / {len(anomalies):,} ({100*anomalies.sum()/len(anomalies):.2f}%)")
