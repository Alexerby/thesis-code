 ...
 * @return A ratio [0.0, 1.0] where > 0.5 indicates buy-side pressure on this
 * venue.
 * */
...
/**
 * Calculates the aggregated order book imbalance across multiple depth levels
 * and venues.
 * * Provides a "Global Deep Book" feature by summing the total bid and ask
 * volume for the top N price levels across all active publishers. Aggregating
 * multiple levels reduces noise from top-of-book "flickering" and helps detect
 * layering-based spoofing strategies.
 * * @param instrument_id The unique identifier for the financial instrument.
 * @param depth The number of price levels (ticks) to aggregate from each side
 * of the book.
 * @return A normalized ratio [0.0, 1.0]. A value of 0.5 indicates a balanced
 * deep book; values nearing 1.0 indicate heavy buy-side concentration across
 * the specified depth.
 */
