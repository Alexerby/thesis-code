...
std::vector<std::string> Visualizer::GetPriceChartLines() {
  ...
  for (const auto &pt : m_price_history) {
    if (pt.ts_nanos < m_start_ts)
      continue;

    double progress =
        static_cast<double>(pt.ts_nanos - m_start_ts) / total_nanos;
  ...
  lines.push_back(std::string(11, ' ') + " " + extract_time(m_start_ts) +
                  std::string(width > 16 ? width - 16 : 0, ' ') +
                  extract_time(m_end_ts));
...
void Visualizer::DrawPriceHistory() {
...
void Visualizer::Render(const MarketState &state) {
  ...
  auto left = GetDashboardLines(state);
  auto right = GetOrderbookLines(state);
  DrawSplitPanel(left, right);
